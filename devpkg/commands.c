...
int Command_fetch(apr_pool_t *p, const char* url, int fetch_only)
{
	...
	if (apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
		...
		rc = Shell_exec(TAR_SH, "FILE", TAR_BZ2_SRC);
	} else {
...
{
	int rc = 0;
	check(access(BUILD_DIR, R_OK | W_OK | X_OK) == 0,
		       	"Build directory doesnt exist: %s", BUILD_DIR);
		...
		const char *make_opts, const char* install_opts)
{
	...
	if (rc == 1) {
		Command_build(p, url, configure_opts, make_opts, install_opts);
		check(rc == 0, "Failed to build %s", url);
	} else if (rc == 0) {
		log_info("Depends successfully installed");
	} else {
		sentinel("Install failed");
	}
