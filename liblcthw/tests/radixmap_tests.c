...
static int test_search(RadixMap *map)
{
	...
	for (i = map -> end / 2; i < map -> end; i++) {
		...
		check(found != NULL, "Didnt find %u at %i", d -> data.key, i);
		check(found -> data.key == d -> data.key, "Got the wrong results: %p: %u looking for %u at %u", 
...
static char* test_operations()
{
	...
	mu_assert(map != NULL, "Failed to create the map");
	...
	mu_assert(check_order(map), "RadixMap didnt stay ordered after search");

	while (map -> end > 0) {
		RMElement *el = RadixMap_find(map, map -> contents[map -> end/2].data.key);
		mu_assert(el != NULL, "Should get a results");
		...
		mu_assert(check_order(map), "RadixMap didnt stay sorted after delete");
	}
...
char *all_tests()
{
	...
	printf("RadixMap performance test: sorted in %f sec \n",
