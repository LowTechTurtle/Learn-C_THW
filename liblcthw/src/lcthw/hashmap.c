...
int Hashmap_set(Hashmap *map, void *key, void *data)
{
	...
	int node_i = Hashmap_get_node(map, hash, bucket, key);
	if (node_i != -1) {
		HashmapNode *node = DArray_get(bucket, node_i);
		check(node != NULL,"Got NULL in getting an existing node");
		node -> key = key;
		node -> data = data;
		node -> hash = hash;
...
void *Hashmap_get(Hashmap *map, void *key)
{
	...
	HashmapNode *node = DArray_get(bucket, i);
	check(node != NULL, "Failed to get node from bucket when it should exists");
